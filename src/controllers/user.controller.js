...
const registerUser = asyncHandler(async(req,res,next)=>{
    ...
    // await deleteAvatarFile(avatarLocalPath);
...
const updateAccountDetails = asyncHandler(async(req,res)=>{
    ...
    const user = await User.findByIdAndUpdate(req.user?.id,{$set:{fullname,email}},{new:true})
    .select("-password");
...
const userAvatarUpdate = asyncHandler(async(req,res)=>{
    ...
    if(!avatarLocalPath){
        throw new ApiError(400,"Avavtar file is missing");
    }
    const avatar = await uploadOnCLoudinary(avatarLocalPath);
...
const userCoverImageUpdate = asyncHandler(async(req,res)=>{
    ...
    const user = await User.findByIdAndUpdate(req.user?.id,{$set:{coverImage:coverImage.url}},{new:true}).select("-password");
...
export {registerUser,loginUser,logoutUser,refreshAccessToken,changeCurrentPassword,getCurrentUser,updateAccountDetails,userAvatarUpdate,userCoverImageUpdate,getUserChannelProfile,getWatchHistory};